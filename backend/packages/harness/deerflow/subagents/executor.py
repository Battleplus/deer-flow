...
class SubagentExecutor:
    ...
    async def _aexecute_admitted(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        try:
            ...
            # Retain the stream so every exit path (completion, cooperative
            # cancellation, graph errors, external cancellation) closes it
            # explicitly. Returning from inside the ``async for`` would leave
            # the graph generator suspended: the terminal result becomes
            # observable and the outer ``finally`` releases the sandbox lease
            # and sends the task-stop notification before graph-stream teardown
            # has completed.
            ...
            try:
                ...
                try:
                    ...
                    # Close-only failures must never mask the primary execution
                    # outcome (result, graph error, or cancellation in flight).
                    logger.warning(
                        "[trace=%s] Subagent %s graph stream close failed (non-fatal)",
                        self.trace_id,
                        self.config.name,
                        exc_info=True,
                    )
